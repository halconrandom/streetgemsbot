...
const main = async () => {
    ...
    try {
      ...
      if (opendiscord.defaults.getDefault("errorHandling")) {
        ...
        if (error.message.toLowerCase().includes("used disallowed intents")) {
          //invalid intents
          opendiscord.log(
            "Open Ticket doesn't work without Privileged Gateway Intents enabled!",
            "error"
          );
          opendiscord.log(
          ...
          error.message
            .toLowerCase()
            .includes("invalid discord bot token provided")
        ) {
          //invalid token
          opendiscord.log(
            "An invalid discord auth token was provided!",
            "error"
          );
          opendiscord.log(
            "Check the config if you have inserted the bot token correctly!",
            "info"
          );
          console.log("\n");
          process.exit(1);
          ...
          await opendiscord.events
            .get("afterErrorHandling")
            .emit([error, origin, errmsg]);
        }
      }
    } catch (err) {
  ...
  const lastVersion = await (
    await import("./core/startup/manageMigration.js")
  ).loadVersionMigrationSystem();
  ...
  await opendiscord.events
    .get("afterPluginsLoaded")
    .emit([opendiscord.plugins]);
  ...
  await opendiscord.events
    .get("onPluginClassLoad")
    .emit([opendiscord.plugins.classes, opendiscord.plugins]);
  await opendiscord.events
    .get("afterPluginClassesLoaded")
    .emit([opendiscord.plugins.classes, opendiscord.plugins]);
  ...
  if (opendiscord.defaults.getDefault("flagInitiating")) {
    ...
    opendiscord.debugfile.writeText(
      "\n[ENABLED FLAGS]:\n" +
        opendiscord.flags
          .getFiltered((flag) => flag.value == true)
          .map((flag) => flag.id.value)
          .join("\n") +
        "\n"
    );
    await opendiscord.events
      .get("afterFlagsInitiated")
      .emit([opendiscord.flags]);
  }
  ...
  if (opendiscord.defaults.getDefault("progressBarRendererLoading")) {
    await (
      await import("./data/framework/progressBarLoader.js")
    ).loadAllProgressBarRenderers();
  }
  await opendiscord.events
    .get("onProgressBarRendererLoad")
    .emit([opendiscord.progressbars.renderers]);
  await opendiscord.events
    .get("afterProgressBarRenderersLoaded")
    .emit([opendiscord.progressbars.renderers]);
  ...
  await opendiscord.events
    .get("onProgressBarLoad")
    .emit([opendiscord.progressbars]);
  await opendiscord.events
    .get("afterProgressBarsLoaded")
    .emit([opendiscord.progressbars]);
  ...
  await opendiscord.events
    .get("afterConfigsLoaded")
    .emit([opendiscord.configs]);
  ...
  if (opendiscord.defaults.getDefault("emojiTitleStyleLoading")) {
    ...
    opendiscord.defaults.setDefault(
      "emojiTitleStyle",
      generalConfig.data.system.emojiStyle
    );
  }
  ...
  //select language
  await opendiscord.events
    .get("onLanguageSelect")
    .emit([opendiscord.languages]);
  if (opendiscord.defaults.getDefault("languageSelection")) {
    //set current language
    const languageId = generalConfig?.data?.language
      ? generalConfig.data.language
      : "english";
    ...
    if (opendiscord.languages.exists(backupLanguageId)) {
      ...
      throw new api.ODSystemError(
        "Unknown backup language '" + backupLanguageId + "'!"
      );

    await opendiscord.events
      .get("afterLanguagesSelected")
      .emit([
        opendiscord.languages.get(languageId),
        opendiscord.languages.get(backupLanguageId),
        opendiscord.languages,
      ]);
  }
  ...
  if (lastVersion)
    await (
      await import("./core/startup/manageMigration.js")
    ).loadAllAfterInitVersionMigrations(lastVersion);

  //load config checker
  opendiscord.log("Loading config checker...", "system");
  if (opendiscord.defaults.getDefault("checkerLoading")) {
    await (
      await import("./data/framework/checkerLoader.js")
    ).loadAllConfigCheckers();
  }
  await opendiscord.events.get("onCheckerLoad").emit([opendiscord.checkers]);
  await opendiscord.events
    .get("afterCheckersLoaded")
    .emit([opendiscord.checkers]);

  //load config checker functions
  if (opendiscord.defaults.getDefault("checkerFunctionLoading")) {
    await (
      await import("./data/framework/checkerLoader.js")
    ).loadAllConfigCheckerFunctions();
  }
  await opendiscord.events
    .get("onCheckerFunctionLoad")
    .emit([opendiscord.checkers.functions, opendiscord.checkers]);
  await opendiscord.events
    .get("afterCheckerFunctionsLoaded")
    .emit([opendiscord.checkers.functions, opendiscord.checkers]);
  ...
  if (opendiscord.defaults.getDefault("checkerExecution")) {
    ...
    await opendiscord.events
      .get("afterCheckersExecuted")
      .emit([result, opendiscord.checkers]);
  }
  ...
  if (opendiscord.defaults.getDefault("checkerTranslationLoading")) {
    await (
      await import("./data/framework/checkerLoader.js")
    ).loadAllConfigCheckerTranslations();
  }
  await opendiscord.events
    .get("onCheckerTranslationLoad")
    .emit([
      opendiscord.checkers.translation,
      generalConfig &&
      generalConfig.data.system &&
      generalConfig.data.system.useTranslatedConfigChecker
        ? generalConfig.data.system.useTranslatedConfigChecker
        : false,
      opendiscord.checkers,
    ]);
  await opendiscord.events
    .get("afterCheckerTranslationsLoaded")
    .emit([opendiscord.checkers.translation, opendiscord.checkers]);
  ...
  await opendiscord.events
    .get("onCheckerRender")
    .emit([opendiscord.checkers.renderer, opendiscord.checkers]);
  if (
    opendiscord.defaults.getDefault("checkerRendering") &&
    !(disableCheckerFlag ? disableCheckerFlag.value : false)
  ) {
    //check if there is a result (otherwise throw minor error)
    const result = opendiscord.checkers.lastResult;
    if (!result)
      return opendiscord.log(
        "Failed to render Config Checker! (couldn't fetch result)",
        "error"
      );

    //get components & check if full mode enabled
    const components = opendiscord.checkers.renderer.getComponents(
      !(advancedCheckerFlag ? advancedCheckerFlag.value : false),
      opendiscord.defaults.getDefault("checkerRenderEmpty"),
      opendiscord.checkers.translation,
      result
    );
    ...
    if (
      ...
      result.messages.every((msg) => msg.type != "error")
    )
      await utilities.timer(5000);

    await opendiscord.events
      .get("afterCheckersRendered")
      .emit([opendiscord.checkers.renderer, opendiscord.checkers]);
  }

  //quit config checker (when required)
  if (
    opendiscord.checkers.lastResult &&
    !opendiscord.checkers.lastResult.valid &&
    !(disableCheckerFlag ? disableCheckerFlag.value : false)
  ) {
  ...
  if (opendiscord.defaults.getDefault("clientLoading")) {
    ...
    const configToken = opendiscord.configs.get("opendiscord:general").data
      .token
      ? opendiscord.configs.get("opendiscord:general").data.token
      : "";
    const envToken = opendiscord.env.getVariable("TOKEN")
      ? opendiscord.env.getVariable("TOKEN")
      : "";
    const token = opendiscord.configs.get("opendiscord:general").data
      .tokenFromENV
      ? envToken
      : configToken;
  ...
  opendiscord.client.readyListener = async () => {
    ...
    if (opendiscord.defaults.getDefault("clientReady")) {
      ...
      if (!mainServer || !client.checkBotInGuild(mainServer)) {
        console.log("\n");
        opendiscord.log(
          "The bot isn't a member of the server provided in the config!",
          "error"
        );
      ...
      if (!client.checkGuildPerms(mainServer)) {
        console.log("\n");
        opendiscord.log(
          "The bot doesn't have the correct permissions in the server provided in the config!",
          "error"
        );
        opendiscord.log(
          'Please give the bot "Administrator" permissions or visit the documentation!',
          "info"
        );
        console.log("\n");
        process.exit(1);
      ...
      if (opendiscord.defaults.getDefault("clientMultiGuildWarning")) {
        ...
        if (botServers.length > 1) {
          opendiscord.log(
            "This bot is part of multiple servers, but Open Ticket doesn't have support for it!",
            "warning"
          );
          opendiscord.log(
            "It may result in the bot crashing & glitching when used in these servers!",
            "info"
          );
        }
        botServers.forEach((server) => {
          ...
          if (!client.checkGuildPerms(server))
            opendiscord.log(
              `The bot doesn't have the correct permissions in the server "${server.name}"!`,
              "warning"
            );
        });
      }
      ...
      if (opendiscord.defaults.getDefault("clientActivityLoading")) {
        ...
        if (generalConfig.data.status && generalConfig.data.status.enabled)
          opendiscord.client.activity.setStatus(
            generalConfig.data.status.type,
            generalConfig.data.status.text,
            generalConfig.data.status.status
          );
      }
      await opendiscord.events
        .get("onClientActivityLoad")
        .emit([opendiscord.client.activity, opendiscord.client]);
      await opendiscord.events
        .get("afterClientActivityLoaded")
        .emit([opendiscord.client.activity, opendiscord.client]);

      //initiate client activity
      await opendiscord.events
        .get("onClientActivityInit")
        .emit([opendiscord.client.activity, opendiscord.client]);
      if (opendiscord.defaults.getDefault("clientActivityInitiating")) {
        opendiscord.client.activity.initStatus();
        await opendiscord.events
          .get("afterClientActivityInitiated")
          .emit([opendiscord.client.activity, opendiscord.client]);
      }
      ...
      if (opendiscord.defaults.getDefault("slashCommandLoading")) {
        await (
          await import("./data/framework/commandLoader.js")
        ).loadAllSlashCommands();
      }
      await opendiscord.events
        .get("onSlashCommandLoad")
        .emit([opendiscord.client.slashCommands, opendiscord.client]);
      await opendiscord.events
        .get("afterSlashCommandsLoaded")
        .emit([opendiscord.client.slashCommands, opendiscord.client]);

      //register slash commands (create, update & remove)
      if (opendiscord.defaults.getDefault("forceSlashCommandRegistration"))
        opendiscord.log(
          "Forcing all slash commands to be re-registered...",
          "system"
        );
      opendiscord.log(
        "Registering slash commands... (this can take up to 2 minutes)",
        "system"
      );
      await opendiscord.events
        .get("onSlashCommandRegister")
        .emit([opendiscord.client.slashCommands, opendiscord.client]);
      if (opendiscord.defaults.getDefault("slashCommandRegistering")) {
        //get all commands that are already registered in the bot
        const cmds =
          await opendiscord.client.slashCommands.getAllRegisteredCommands();
        const removableCmds = cmds.unused.map((cmd) => cmd.cmd);
        const newCmds = cmds.unregistered.map((cmd) => cmd.instance);
        const updatableCmds = cmds.registered
          .filter(
            (cmd) =>
              cmd.requiresUpdate ||
              opendiscord.defaults.getDefault("forceSlashCommandRegistration")
          )
          .map((cmd) => cmd.instance);

        //init progress bars
        const removeProgress = opendiscord.progressbars.get(
          "opendiscord:slash-command-remove"
        );
        const createProgress = opendiscord.progressbars.get(
          "opendiscord:slash-command-create"
        );
        const updateProgress = opendiscord.progressbars.get(
          "opendiscord:slash-command-update"
        );

        //remove unused cmds, create new cmds & update existing cmds
        if (opendiscord.defaults.getDefault("allowSlashCommandRemoval"))
          await opendiscord.client.slashCommands.removeUnusedCommands(
            removableCmds,
            undefined,
            removeProgress
          );
        await opendiscord.client.slashCommands.createNewCommands(
          newCmds,
          createProgress
        );
        await opendiscord.client.slashCommands.updateExistingCommands(
          updatableCmds,
          updateProgress
        );

        await opendiscord.events
          .get("afterSlashCommandsRegistered")
          .emit([opendiscord.client.slashCommands, opendiscord.client]);
      }
      ...
      await opendiscord.events
        .get("onTextCommandLoad")
        .emit([opendiscord.client.textCommands, opendiscord.client]);
      await opendiscord.events
        .get("afterTextCommandsLoaded")
        .emit([opendiscord.client.textCommands, opendiscord.client]);

      //client ready
      await opendiscord.events
        .get("afterClientReady")
        .emit([opendiscord.client]);
    }
  };
  ...
  if (opendiscord.defaults.getDefault("clientInitiating")) {
    ...
    await opendiscord.events
      .get("afterClientInitiated")
      .emit([opendiscord.client]);

    //client login
    await opendiscord.client
      .login()
      .catch((reason) =>
        process.emit("uncaughtException", new api.ODSystemError(reason))
      );
  ...
  if (opendiscord.defaults.getDefault("questionLoading")) {
    await (
      await import("./data/openticket/questionLoader.js")
    ).loadAllQuestions();
  }
  await opendiscord.events.get("onQuestionLoad").emit([opendiscord.questions]);
  await opendiscord.events
    .get("afterQuestionsLoaded")
    .emit([opendiscord.questions]);
  ...
  await opendiscord.events
    .get("afterOptionsLoaded")
    .emit([opendiscord.options]);
  ...
  await opendiscord.events
    .get("afterTicketsLoaded")
    .emit([opendiscord.tickets]);
  ...
  if (opendiscord.defaults.getDefault("blacklistLoading")) {
    await (
      await import("./data/openticket/blacklistLoader.js")
    ).loadAllBlacklistedUsers();
  }
  await opendiscord.events.get("onBlacklistLoad").emit([opendiscord.blacklist]);
  await opendiscord.events
    .get("afterBlacklistLoaded")
    .emit([opendiscord.blacklist]);
  ...
  if (opendiscord.defaults.getDefault("transcriptCompilerLoading")) {
    await (
      await import("./data/openticket/transcriptLoader.js")
    ).loadAllTranscriptCompilers();
  }
  await opendiscord.events
    .get("onTranscriptCompilerLoad")
    .emit([opendiscord.transcripts]);
  await opendiscord.events
    .get("afterTranscriptCompilersLoaded")
    .emit([opendiscord.transcripts]);
  ...
  await opendiscord.events
    .get("onTranscriptHistoryLoad")
    .emit([opendiscord.transcripts]);
  await opendiscord.events
    .get("afterTranscriptHistoryLoaded")
    .emit([opendiscord.transcripts]);
  ...
  await opendiscord.events
    .get("onButtonBuilderLoad")
    .emit([
      opendiscord.builders.buttons,
      opendiscord.builders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterButtonBuildersLoaded")
    .emit([
      opendiscord.builders.buttons,
      opendiscord.builders,
      opendiscord.actions,
  ...
  await opendiscord.events
    .get("onDropdownBuilderLoad")
    .emit([
      opendiscord.builders.dropdowns,
      opendiscord.builders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterDropdownBuildersLoaded")
    .emit([
      opendiscord.builders.dropdowns,
      opendiscord.builders,
      opendiscord.actions,
  ...
  await opendiscord.events
    .get("onFileBuilderLoad")
    .emit([
      opendiscord.builders.files,
      opendiscord.builders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterFileBuildersLoaded")
    .emit([
      opendiscord.builders.files,
      opendiscord.builders,
      opendiscord.actions,
  ...
  await opendiscord.events
    .get("onEmbedBuilderLoad")
    .emit([
      opendiscord.builders.embeds,
      opendiscord.builders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterEmbedBuildersLoaded")
    .emit([
      opendiscord.builders.embeds,
      opendiscord.builders,
      opendiscord.actions,
  ...
  await opendiscord.events
    .get("onMessageBuilderLoad")
    .emit([
      opendiscord.builders.messages,
      opendiscord.builders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterMessageBuildersLoaded")
    .emit([
      opendiscord.builders.messages,
      opendiscord.builders,
      opendiscord.actions,
  ...
  await opendiscord.events
    .get("onModalBuilderLoad")
    .emit([
      opendiscord.builders.modals,
      opendiscord.builders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterModalBuildersLoaded")
    .emit([
      opendiscord.builders.modals,
      opendiscord.builders,
      opendiscord.actions,
  ...
  if (opendiscord.defaults.getDefault("commandRespondersLoading")) {
    ...
    await (
      await import("./commands/autodelete.js")
    ).registerCommandResponders();
  }
  await opendiscord.events
    .get("onCommandResponderLoad")
    .emit([
      opendiscord.responders.commands,
      opendiscord.responders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterCommandRespondersLoaded")
    .emit([
      opendiscord.responders.commands,
      opendiscord.responders,
      opendiscord.actions,
  ...
  if (opendiscord.defaults.getDefault("buttonRespondersLoading")) {
    await (
      await import("./actions/handleVerifyBar.js")
    ).registerButtonResponders();
    await (
      await import("./actions/handleTranscriptErrors.js")
    ).registerButtonResponders();
  ...
  await opendiscord.events
    .get("onButtonResponderLoad")
    .emit([
      opendiscord.responders.buttons,
      opendiscord.responders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterButtonRespondersLoaded")
    .emit([
      opendiscord.responders.buttons,
      opendiscord.responders,
      opendiscord.actions,
  ...
  await opendiscord.events
    .get("onDropdownResponderLoad")
    .emit([
      opendiscord.responders.dropdowns,
      opendiscord.responders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterDropdownRespondersLoaded")
    .emit([
      opendiscord.responders.dropdowns,
      opendiscord.responders,
      opendiscord.actions,
  ...
  await opendiscord.events
    .get("onModalResponderLoad")
    .emit([
      opendiscord.responders.modals,
      opendiscord.responders,
      opendiscord.actions,
    ]);
  await opendiscord.events
    .get("afterModalRespondersLoaded")
    .emit([
      opendiscord.responders.modals,
      opendiscord.responders,
      opendiscord.actions,
  ...
  if (opendiscord.defaults.getDefault("actionsLoading")) {
    await (
      await import("./actions/createTicketPermissions.js")
    ).registerActions();
  ...
  await opendiscord.events
    .get("afterActionsLoaded")
    .emit([opendiscord.actions]);
  ...
  await opendiscord.events
    .get("onVerifyBarLoad")
    .emit([opendiscord.verifybars]);
  await opendiscord.events
    .get("afterVerifyBarsLoaded")
    .emit([opendiscord.verifybars]);
  ...
  if (opendiscord.defaults.getDefault("permissionsLoading")) {
    await (
      await import("./data/framework/permissionLoader.js")
    ).loadAllPermissions();
  }
  await opendiscord.events
    .get("onPermissionLoad")
    .emit([opendiscord.permissions]);
  await opendiscord.events
    .get("afterPermissionsLoaded")
    .emit([opendiscord.permissions]);
  ...
  if (opendiscord.defaults.getDefault("postsInitiating")) {
    if (opendiscord.client.mainServer)
      opendiscord.posts.init(opendiscord.client.mainServer);
    await opendiscord.events
      .get("afterPostsInitiated")
      .emit([opendiscord.posts]);
  }
  ...
  if (opendiscord.defaults.getDefault("cooldownsLoading")) {
    await (
      await import("./data/framework/cooldownLoader.js")
    ).loadAllCooldowns();
  }
  await opendiscord.events.get("onCooldownLoad").emit([opendiscord.cooldowns]);
  await opendiscord.events
    .get("afterCooldownsLoaded")
    .emit([opendiscord.cooldowns]);
  ...
  if (opendiscord.defaults.getDefault("helpMenuCategoryLoading")) {
    await (
      await import("./data/framework/helpMenuLoader.js")
    ).loadAllHelpMenuCategories();
  }
  await opendiscord.events
    .get("onHelpMenuCategoryLoad")
    .emit([opendiscord.helpmenu]);
  await opendiscord.events
    .get("afterHelpMenuCategoriesLoaded")
    .emit([opendiscord.helpmenu]);
  ...
  if (opendiscord.defaults.getDefault("helpMenuComponentLoading")) {
    await (
      await import("./data/framework/helpMenuLoader.js")
    ).loadAllHelpMenuComponents();
  }
  await opendiscord.events
    .get("onHelpMenuComponentLoad")
    .emit([opendiscord.helpmenu]);
  await opendiscord.events
    .get("afterHelpMenuComponentsLoaded")
    .emit([opendiscord.helpmenu]);

  //load stat scopes
  opendiscord.log("Loading stats...", "system");
  if (opendiscord.defaults.getDefault("statScopesLoading")) {
    opendiscord.stats.useDatabase(
      opendiscord.databases.get("opendiscord:stats")
    );
  ...
  await opendiscord.events
    .get("afterStatScopesLoaded")
    .emit([opendiscord.stats]);
  ...
  if (opendiscord.defaults.getDefault("liveStatusLoading")) {
    await (
      await import("./data/framework/liveStatusLoader.js")
    ).loadAllLiveStatusSources();
  }
  await opendiscord.events
    .get("onLiveStatusSourceLoad")
    .emit([opendiscord.livestatus]);
  await opendiscord.events
    .get("afterLiveStatusSourcesLoaded")
    .emit([opendiscord.livestatus]);
  ...
  if (opendiscord.defaults.getDefault("startScreenLoading")) {
    await (
      await import("./data/framework/startScreenLoader.js")
    ).loadAllStartScreenComponents();
  }
  await opendiscord.events
    .get("onStartScreenLoad")
    .emit([opendiscord.startscreen]);
  await opendiscord.events
    .get("afterStartScreensLoaded")
    .emit([opendiscord.startscreen]);

  //render startscreen
  await opendiscord.events
    .get("onStartScreenRender")
    .emit([opendiscord.startscreen]);
  if (opendiscord.defaults.getDefault("startScreenRendering")) {
    ...
    if (opendiscord.languages.getLanguageMetadata(false)?.automated) {
      console.log("===================");
      opendiscord.log(
        "You are currently using a language which has been translated by Google Translate!",
        "warning"
      );
      opendiscord.log(
        "Please help us improve the translation by contributing to our project!",
        "warning"
      );
      console.log("===================");
    }
